  ...
  const handleFormSubmit = async (e) => {
    ...
    try {  
      ...
      if (response.ok) {
        ...
        navigate(from); // Use navigate instead of window.location.href
      } else {
        const errorData = await response.json();
        toast.error(errorData.message || "Login failed. Please try again.", { theme: "dark" }); 
      }
    } catch (error) {
      console.error('Error submitting form:', error);
      toast.error("Failed to submit form. Please try again later.", { theme: "dark" }); 
    } finally {
      setLoading(false); // Move this to finally block
    }
  };
                ...
                <div className="card p-2">
                  ...
                  <form onSubmit={handleFormSubmit} className='form-group'>
                    <div className='input-group'>
                      ...
                      <input
                        type="email"
                        name="email" // Change to match the state key
                        required  
                        placeholder="Email Address"
                        value={formValues.email} // Change to formValues.email
                    ...
                    <div className="text-end"> 
                      <Link to="/forgot" className="text-light">Forgot Password?</Link>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="btn btn-block my-3 text-capitalize text-light" onClick={login}> 
                        <img src="img/Google.png" alt="google" className='img-fluid me-2' />Login With Google
                      </div>
                    </div>
                  </form> 
                  <div className="d-flex my-3">
                    <p>Don't have an account?</p> 
                    <Link to="/signup" className="ps-2 text-light text-decoration-underline">Sign Up</Link>
                  </div>
